...
save_rate = 4
random.seed(4)
full_dataset = ucf_c3d('video_annotations_resnet','G:\\C3D Data', kernels=['four_frames', 'diff'])
...
save_path = 'epoch12_accuracy0.01426962072850169.dict'
...
optimizer = optim.SGD(model.parameters(), lr=.002, momentum=0.5)
...
def train_batch(model, optimizer, videos_four_frames, videos_diff, labels):
    '''
    :param videos:n*segments, channels, frames, height, width
    :param labels:n*101 1/0
    '''
    optimizer.zero_grad()
    y = model(videos_four_frames.cuda(), videos_diff.cuda())
...
for curr_epoch in range(epochs):
    ...
    for i_batch, sample_batched in tqdm(enumerate(train_dataloader)):
        ...
        action_list = sample_batched['action']
        labels = torch.tensor(action2label(list(action_list), auxdict), dtype=torch.long).cuda()
        batch_loss = train_batch(model, optimizer, video_matrix_four, video_matrix_diff, labels)
